...
| `windy_point_forecast` | Windy.com Point Forecast API v2 | `https://api.windy.com/api/point-forecast/v2` | Handled in `server.js` `/api/windy/point-forecast` | `POST /api/windy/point-forecast`, `GET /api/windy/config` | Per-call | Required: `WINDY_API_KEY` (or `WINDY_MAP_KEY`) | Citizen Windy Weather Radar drawer, timeline forecast series |
